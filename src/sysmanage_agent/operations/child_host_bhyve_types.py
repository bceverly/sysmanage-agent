"""
bhyve type definitions for child host operations on FreeBSD.
"""
...
@dataclass
class BhyveVmConfig:  # pylint: disable=too-many-instance-attributes
    """Configuration for creating a bhyve virtual machine."""
    ...
    memory: str = "1G"  # Memory allocation (e.g., "1G", "2048M")
    ...
    use_uefi: bool = True  # Use UEFI firmware (required for non-FreeBSD guests)
    auto_approve_token: Optional[str] = None  # Token for auto-approval
    child_host_id: Optional[str] = None  # ID for status updates

    # bhyve-specific paths
    vm_dir: str = "/vm"  # Base directory for VM storage
    zvol_parent: str = "zroot/vm"  # ZFS parent for zvols (if using ZFS)
    use_zvol: bool = False  # Use ZFS zvol for disk (auto-detected)
