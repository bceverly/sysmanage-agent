...
from src.sysmanage_agent.core.agent_lifecycle import AgentLifecycleMixin
...
class SysManageAgent(
    AgentDelegatorMixin, AgentLifecycleMixin
    ...
    async def run(self):
        ...
        system_info = self.registration.get_system_info()
        self._log_startup_banner(system_info)
        ...
        self._log_certificate_status()

        self.logger.info("Starting WebSocket connection...")
        await self._connection_loop(self.config.get_reconnect_interval())

    async def _connection_loop(self, base_reconnect_interval):
        """Stay connected, by WebSocket where possible and polling where not.

        Returns when the reconnect handler says to stop; every other path loops.
        """
        while True:
