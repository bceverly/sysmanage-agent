...
help:
	...
	@echo ""
	@echo "i18n / translation (gettext .po/.mo):"
	@echo "  make i18n-fix          - THE fix-it command: extract, merge, requeue, translate, compile, re-verify."
	@echo "                           Needs the GPU service: make i18n-fix SERVICE=http://<host>:8765"
	@echo "  make translate         - Fill empty msgstr from the GPU service"
	...
	@echo "  make i18n-validate     - Every _() msgid in code is translated; fuzzy budget respected"
	@echo "  make i18n-extract      - Scan source _() strings into messages.pot"
	@echo "  make i18n-merge        - Merge .pot into locale .po (adds msgids, clears fuzzy, seeds English)"
	@echo "  make i18n-compile      - Compile .po into the runtime .mo (installers depend on this)"
	@echo "  make i18n-strip-fuzzy  - Turn every fuzzy entry into a clean untranslated gap"
	...
	@echo "  make i18n-check-coverage - Gate gettext coverage"
	@echo "  make i18n-check-msgid-style - Gate the SHAPE of msgids"
	@echo ""
	...
	@echo "  make check-syntax      - Quick syntax-only check of the source tree"
	@echo ""
	@echo "Setup, dev and test helpers:"
	...
	@echo "  make run-dev           - Run the agent in development mode (verbose)"
	@echo "  make test-quick        - Run a single test file quickly"
	@echo "  make coverage          - Test coverage report"
	@echo "  make install-dev-rpm   - Install RPM build deps (CentOS/RHEL/Fedora)"
	@echo "  make install-dev-rpm-suse - Install RPM build deps (openSUSE/SLES)"
	@echo ""
	@echo "More packaging:"
	@echo "  make installer-pkg     - Build the macOS .pkg installer"
	@echo "  make installer-rpm-suse - Build the openSUSE/SLES .rpm"
	@echo "  make snap-strict-publish - Publish the strict snap to the store (edge channel)"
	@echo "  make flatpak           - Build the Flatpak package"
	@echo "  make flatpak-install / flatpak-uninstall / flatpak-clean - Flatpak lifecycle"
	@echo ""
...
deploy-copr:
	...
	pip3 download pip -d /tmp/vendor --only-binary=:all:; \
	echo "Downloading wheels for Python 3.9 x86_64 (EPEL 9, CentOS Stream 9, Amazon Linux 2023)..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.9.21 \
		--platform manylinux2014_x86_64 \
		--platform manylinux_2_17_x86_64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.9 aarch64..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.9.21 \
		--platform manylinux2014_aarch64 \
		--platform manylinux_2_17_aarch64 \
	...
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		...
		--platform manylinux_2_17_x86_64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.11 aarch64..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.11.11 \
		--platform manylinux2014_aarch64 \
		--platform manylinux_2_17_aarch64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.12 x86_64 (EPEL 10, CentOS Stream 10)..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.12.11 \
		--platform manylinux2014_x86_64 \
		--platform manylinux_2_17_x86_64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.12 aarch64..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.12.11 \
		--platform manylinux2014_aarch64 \
		--platform manylinux_2_17_aarch64 \
	...
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		...
		--platform manylinux_2_17_x86_64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.13 aarch64..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.13.1 \
		--platform manylinux2014_aarch64 \
		--platform manylinux_2_17_aarch64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.14 x86_64 (Fedora 43)..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.14.0 \
		--platform manylinux2014_x86_64 \
		--platform manylinux_2_17_x86_64 \
		--only-binary=:all:; \
	echo "Downloading wheels for Python 3.14 aarch64..."; \
	pip3 download -r "$$WORKSPACE/requirements-prod.txt" -d /tmp/vendor \
		--python-version 3.14.0 \
		--platform manylinux2014_aarch64 \
		--platform manylinux_2_17_aarch64 \
